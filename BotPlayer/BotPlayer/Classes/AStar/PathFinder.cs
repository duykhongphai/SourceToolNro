...
using System.Linq;
using BotPlayer.Classes.AStar.Collections.PathFinder;
...
public class PathFinder : IFindAPath
{
    ...
    public Position[] FindPath(Position start, Position end)
    {
        ...
        while (graph.HasOpenNodes)
        {
            ...
            if (q.Position == end) return OrderClosedNodesAsArray(graph, q);

            if (nodesVisited > _options.SearchLimit) return new Position[0];

            foreach (var successor in graph.GetSuccessors(q))
            {
                if (_world[successor.Position] == ClosedValue) continue;

                var newG = q.G + DistanceBetweenNodes;

                if (_options.PunishChangeDirection) newG += CalculateModifierToG(q, successor, end);
                ...
                if (BetterPathToSuccessorFound(updatedSuccessor, successor)) graph.OpenNode(updatedSuccessor);
            }
    ...
    {
        if (q.Position == q.ParentNodePosition) return 0;
        ...
        if (successorIsVerticallyAdjacentToQ)
        {
            ...
            if (qIsVerticallyAdjacentToParent) return gPunishment;
        }
        ...
        if (successorIsHorizontallyAdjacentToQ)
        {
            ...
            if (qIsHorizontallyAdjacentToParent) return gPunishment;
        }

        if (_options.UseDiagonals)
        {
            var successorIsDiagonallyAdjacentToQ = successor.Position.Column - successor.Position.Row ==
                                                   q.Position.Column - q.Position.Row;
            if (successorIsDiagonallyAdjacentToQ)
            {
                var qIsDiagonallyAdjacentToParent = q.Position.Column - q.Position.Row ==
                                                    q.ParentNodePosition.Column - q.ParentNodePosition.Row
                                                    && IsStraightLine(q.ParentNodePosition, q.Position,
                                                        successor.Position);
                if (qIsDiagonallyAdjacentToParent) return gPunishment;
            }
        }
